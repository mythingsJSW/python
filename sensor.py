...
def rotate_and_visualize(angles, ranges, rotation_angle_input, angle_offset):
    """
    LiDAR 데이터를 기본 회전 후 양쪽 데이터를 추가 회전하고 시각화.

    Parameters:
        angles (list of float): 센서에서 얻은 각도 데이터 (도 단위).
        ranges (list of float): 센서에서 얻은 거리 데이터.
        rotation_angle_input (float): 전체 데이터를 기본적으로 회전시킬 각도.
        angle_offset (float): 오른쪽과 왼쪽 데이터를 추가로 회전시킬 각도.
    """
    # 극좌표 -> 데카르트 좌표 변환
    x_coords = []
    y_coords = []
    for angle, r in zip(angles, ranges):
        # 기본 회전 적용 (rotation_angle_input)
    ...
    # 오른쪽 데이터를 추가 회전 (X 음의 방향으로 angle_offset)
    rotated_right_x, rotated_right_y = [], []
    for x, y in zip(right_x, right_y):
        x_rot, y_rot = rotate_point(x, y, -angle_offset)  # 시계 방향 추가 회전
        rotated_right_x.append(x_rot)
        rotated_right_y.append(y_rot)

    # 왼쪽 데이터를 추가 회전 (X 양의 방향으로 angle_offset)
    rotated_left_x, rotated_left_y = [], []
    for x, y in zip(left_x, left_y):
        x_rot, y_rot = rotate_point(x, y, angle_offset)  # 반시계 방향 추가 회전
    ...
    # 회전된 왼쪽 데이터
    plt.scatter(rotated_left_x, rotated_left_y, c='blue', label='Left Data (Rotated)')

    # 회전된 오른쪽 데이터
    plt.scatter(rotated_right_x, rotated_right_y, c='green', label='Right Data (Rotated)')
    ...
    # 축 설정
    plt.title(f"LiDAR Data Rotated by {rotation_angle_input}° with Offset {angle_offset}°")
    ...
    # 축 범위
    max_range = max(max(abs(x) for x in x_coords), max(abs(y) for y in y_coords))
    margin = 0.2 * max_range
    plt.xlim(-max_range - margin, max_range + margin)
    plt.ylim(-max_range - margin, max_range + margin)

    # 그래프 저장
    output_file = "lidar_with_input_and_offset.png"
    plt.savefig(output_file)
    print(f"조정된 데이터가 {output_file}에 저장되었습니다.")
...
if __name__ == "__main__":
    ...
    # 회전 각도 입력
    rotation_angle_input = 60.0
    angle_offset = float(input("오른쪽과 왼쪽 데이터를 추가로 회전시킬 각도(도 단위)를 입력하세요: "))
    
    # 함수 호출
    rotate_and_visualize(angles, ranges, rotation_angle_input, angle_offset)
